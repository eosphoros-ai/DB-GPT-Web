      ...
      <div className="border-t-[1px] border-gray-300 mt-3 py-2">
        ...
        {references.map((reference: Reference, index: number) => (
          <p key={`file_${index}`} className="text-sm font-normal block ml-2 h-6 leading-6 overflow-hidden">
            ...
            {reference?.chunks?.map((chunkId, index) => (
              <>
                <Popover content={<ChunkContent chunkId={chunkId} />} title="Chunk Content">
                  <span className="cursor-pointer text-blue-500 ml-2" key={`file_page_${index}`}>
                    {chunkId}
                  </span>
                </Popover>
                {index < reference?.chunks.length - 1 && <span key={`file_page__${index}`}>,</span>}
              </>
            ))}
          </p>
        ))}
