...
const PromptModal = ({ children, prompt_type, item, mutate, setTableParams, pageSize }: any) => {
  ...
  const submit = async () => {
    try {
      ...
      const params = { user_name: userInfo.loginName, prompt_type, ...values }
      setLoading(true)
      if (item) { // 编辑
  ...
  return (
    <>
      <div className='inline-block' onClick={onOpenModal}>{children}</div>
      <Modal
        title={item ? '编辑 Prompts' : '新建 Prompts'}
        open={show}
      ...
      >
        <Spin spinning={loading}>
          <Form
            form={form}
            name={`prompt-item-${item?.prompt_name || 'new'}`}
            labelCol={{ span: 4 }}
            wrapperCol={{ span: 20 }}
            onFinish={submit}
          >
