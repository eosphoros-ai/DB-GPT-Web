...
function DbEditor() {
  ...
  const [autoExpandParent, setAutoExpandParent] = React.useState(true);
	...
	const { data: rounds, loading: roundsLoading } = useRequest(async () => await sendGetRequest('/v1/editor/sql/rounds', {
		con_uid: id
	}), {
		onSuccess: (res) => {
	...
	const { run: runSql, loading: runLoading } = useRequest(async () => {
	...
	const { run: runCharts, loading: runChartsLoading } = useRequest(async () => {
		const db_name = rounds?.data?.find(item => item.round === currentRound)?.db_name;
		const params: {
			db_name: string;
			sql?: string;
	...
	const { run: submitChart, loading: submitChartLoading } = useRequest(async () => {
		const db_name = rounds?.data?.find(item => item.round === currentRound)?.db_name;
